 ...
 */
function findSolarTermJD(targetLon: number, approxJD: number): number {
  let jd = approxJD;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (Math.abs(diff) < 0.0001) break; // converged (~1 second accuracy)
...
function applyDST(date: Date, isDST: boolean): Date {
  if (!isDST) return date;
