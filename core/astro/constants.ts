...
import type {
  HeavenlyStem, EarthlyBranch, HiddenStem,
  HeavenlyStemIndex, EarthlyBranchIndex,
  Wuxing, YinYang, ShiShen, SexagenaryIndex,
} from './types.js';

// ---- Heavenly Stems ----

const STEM_NAMES = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'] as const;
const STEM_WUXING: Wuxing[] = ['木', '木', '火', '火', '土', '土', '金', '金', '水', '水'];
const STEM_YINYANG: YinYang[] = ['阳', '阴', '阳', '阴', '阳', '阴', '阳', '阴', '阳', '阴'];

export function getStem(index: HeavenlyStemIndex): HeavenlyStem {
  return {
    index,
...
export const ALL_STEMS: HeavenlyStem[] = Array.from({ length: 10 }, (_, i) =>
  getStem(i as HeavenlyStemIndex),
);
...
export function getBranch(index: EarthlyBranchIndex): EarthlyBranch {
  return {
    index,
...
export const ALL_BRANCHES: EarthlyBranch[] = Array.from({ length: 12 }, (_, i) =>
  getBranch(i as EarthlyBranchIndex),
);
 ...
 */
export function sexagenaryIndex(stem: number, branch: number): SexagenaryIndex {
...
export function stemFromSexagenary(idx: SexagenaryIndex): HeavenlyStemIndex {
  return (idx % 10) as HeavenlyStemIndex;
}

/** Get branch index from sexagenary index */
export function branchFromSexagenary(idx: SexagenaryIndex): EarthlyBranchIndex {
  return (idx % 12) as EarthlyBranchIndex;
}

/** All 60 sexagenary cycle names */
export const SEXAGENARY_NAMES: string[] = Array.from({ length: 60 }, (_, i) => {
...
export const HIDDEN_STEMS: HiddenStemsMap = {
...
export function getHiddenStems(branch: EarthlyBranchIndex): HeavenlyStemIndex[] {
  return HIDDEN_STEMS[branch].map(h => h.stem as HeavenlyStemIndex);
}

// ---- Nayin (纳音) ----

const NAYIN_PAIRS: string[] = [
...
const NAYIN_TABLE: string[] = NAYIN_PAIRS.flatMap(n => [n, n]);

export function getNayin(sexagenaryIdx: SexagenaryIndex): string {
...
/** 五行相生: 我生者 */
const WUXING_SHENG_BY: Record<Wuxing, Wuxing> = {
  '水': '木',  // 水生木
  '木': '火',  // 木生火
  '火': '土',  // 火生土
  '土': '金',  // 土生金
...
/** 五行相克: 我克者 */
const WUXING_KE_BY: Record<Wuxing, Wuxing> = {
  '金': '木',  // 金克木
  '水': '火',  // 水克火
  '木': '土',  // 木克土
  '火': '金',  // 火克金
 ...
 * target: 目标天干索引
 */
export function getShiShen(dayMaster: HeavenlyStemIndex, target: HeavenlyStemIndex): ShiShen {
 ...
 * 丁壬年 → 壬寅(8), 戊癸年 → 甲寅(0)
 */
const YEAR_STEM_TO_MONTH_STEM_START: Record<number, number> = {
  0: 2, 5: 2,   // 甲己 → 丙(2)
  1: 4, 6: 4,   // 乙庚 → 戊(4)
  2: 6, 7: 6,   // 丙辛 → 庚(6)
  3: 8, 8: 8,   // 丁壬 → 壬(8)
...
export function getMonthStemStart(yearStem: HeavenlyStemIndex): HeavenlyStemIndex {
  return YEAR_STEM_TO_MONTH_STEM_START[yearStem] as HeavenlyStemIndex;
}
 ...
 * 丁壬日 → 庚子(6), 戊癸日 → 壬子(8)
 */
const DAY_STEM_TO_HOUR_STEM_START: Record<number, number> = {
  0: 0, 5: 0,   // 甲己 → 甲(0)
  1: 2, 6: 2,   // 乙庚 → 丙(2)
  2: 4, 7: 4,   // 丙辛 → 戊(4)
  3: 6, 8: 6,   // 丁壬 → 庚(6)
...
export function getHourStemStart(dayStem: HeavenlyStemIndex): HeavenlyStemIndex {
  return DAY_STEM_TO_HOUR_STEM_START[dayStem] as HeavenlyStemIndex;
}
...
/** 每个节气对应的太阳黄经 (0-360°) */
export const JIEQI_LONGITUDE: number[] = [
...
export const JIE_TO_MONTH_BRANCH: Record<number, EarthlyBranchIndex> = {
  0:  1,  // 小寒 → 丑月
  2:  2,  // 立春 → 寅月
  4:  3,  // 惊蛰 → 卯月
  6:  4,  // 清明 → 辰月
