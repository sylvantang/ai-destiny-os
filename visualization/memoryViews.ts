 ...
 */
export function renderLifeTimeline(store: MemoryStore): string {
  const timeline = getLifeTimeline(store);
  if (timeline.length === 0) return '暂无生活事件记录。';
 ...
 */
export function renderAccuracyReport(store: MemoryStore): string {
...
function accuracyStars(score: number): string {
  if (score >= 1.5) return '★★★★★';
  if (score >= 1.0) return '★★★★☆';
  if (score >= 0.5) return '★★★☆☆';
  if (score >= 0.0) return '★★☆☆☆';
 ...
 */
export function renderPatterns(store: MemoryStore): string {
  const patterns = detectPatterns(store);
  if (patterns.length === 0) return '暂无足够数据发现规律。需要至少两个同领域事件。';
 ...
 */
export function renderEventsByDomain(
  store: MemoryStore,
  domain: LifeDomain,
): string {
  const events = store.getEvents({ domain });
  if (events.length === 0) return `${domain}领域暂无记录事件。`;
 ...
 */
export function renderPendingVerifications(store: MemoryStore): string {
 ...
 */
export function renderMemoryViews(store: MemoryStore): string {
