 ...
 */
export function analyzePersonality(ctx) {
...
// ---- Trait Derivation ----
function deriveCoreTraits(chart) {
...
function deriveMBTITendency(chart, strength, structure) {
    const wx = chart.dayMasterWuxing;
    const yy = chart.dayMaster.yinYang;
...
function deriveWorkStyle(_chart, _structure, relations) {
    ...
    if (hasFood && hasWealth)
        return '创意变现型 — 擅长将想法转化为实际价值，适合自主创业';
    if (hasOfficer && hasWealth)
        return '管理执行型 — 善于在规范体系中积累资源和地位';
    if (hasFood)
        return '独立思考型 — 需要创作空间，不喜欢被过度管控';
    if (hasOfficer)
        return '规则导向型 — 在组织结构中发挥最佳，执行力强';
    return '灵活适应型 — 能根据环境调整工作方式，多面手特质';
}
function deriveStressResponse(chart, strength) {
    const wx = chart.dayMasterWuxing;
...
function deriveStrengths(_chart, structure, relations) {
    ...
    if (fav.some(r => r.name.includes('官印')))
        strengths.push('善于整合资源和贵人力量');
    if (fav.some(r => r.name.includes('食伤生财')))
        strengths.push('擅长将才华转化为收入');
    if (strengths.length === 0) {
        strengths.push('综合能力均衡', '适应性强', '可塑性强');
    }
    return strengths.slice(0, 5);
}
function deriveGrowthAreas(_chart, strength, _structure, relations) {
    ...
    if (strength.level === '偏弱')
        areas.push('建立自信，学会拒绝');
    if (strength.level === '偏旺')
        areas.push('培养耐心，避免冲动决策');
    if (areas.length === 0) {
        areas.push('持续自我提升', '拓展社交圈', '培养多元化技能');
    }
    return areas.slice(0, 5);
}
function deriveDecisionStyle(chart, structure) {
    const wx = chart.dayMasterWuxing;
    const pattern = structure.primaryPattern;
    const wxStyle = {
        '木': '凭直觉和价值观决策，重视长期影响',
    ...
    if (pattern.includes('官'))
        style += '，倾向于遵守规则和流程';
    if (pattern.includes('伤'))
        style += '，倾向于打破常规寻找新方案';
    return style;
}
function deriveSocialPattern(_chart, relations) {
...
function patternNote(pattern, subPattern, patternShiShen) {
...
function relationNote(relations) {
 ...
 */
export function renderPersonalityProse(result, ctx) {
    const { strength, structure, relations } = ctx;
    const dm = strength.dayMaster;
    const wx = dm.wuxing;
    const paragraphs = [];
    // 1. Opening + day master introduction
    paragraphs.push(`我们先从你的日主说起。你是${dm.stem}${wx}日主，属${dm.yinYang}性。` +
        (WX_NATURE[wx] ?? '') +
        '。' +
    ...
    // 7. Social pattern
    paragraphs.push(`在社交中，${result.socialPattern}。` +
        `工作风格上，${result.workStyle}。` +
        `面对压力时，${result.stressResponse}`);
    // 8. Closing
    paragraphs.push('认识自己的命盘，不是给自己贴标签，而是理解自己的"出厂设置"。' +
