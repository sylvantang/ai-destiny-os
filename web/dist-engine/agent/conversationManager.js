 ...
 */
export function createConversation(agent) {
    return {
        agent,
...
function buildInitialContext(agent) {
    const { ctx } = agent.state;
    return [{
            role: 'system',
            content: [
 ...
 */
export function processTurnWithMemory(state, input) {
...
function generateSuggestions(topicsCovered) {
 ...
 */
export function getSuggestionsForDomain(domain) {
