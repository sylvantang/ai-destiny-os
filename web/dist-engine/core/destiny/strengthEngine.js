...
import { isClash, isCombination, COMBINATION_WUXING, getPunishment, isHarm, THREE_HARMONY_SETS, getHalfHarmony, THREE_MEETING_SETS, } from '../astro/earthlyBranchRelations.js';
/** Season ruler element: 春木 夏火 秋金 冬水 */
function getSeasonRuler(monthBranch) {
    // 寅卯辰=春→木, 巳午未=夏→火, 申酉戌=秋→金, 亥子丑=冬→水
    if (monthBranch >= 2 && monthBranch <= 4)
        return '木';
    if (monthBranch >= 5 && monthBranch <= 7)
        return '火';
    if (monthBranch >= 8 && monthBranch <= 10)
        return '金';
    return '水';
}
/**
 ...
 */
function getSeasonalState(dmWx, monthBranch) {
...
// ---- Main ----
export function analyzeStrength(bazi, climate) {
    const dm = bazi.day.stemIndex;
...
function buildClimateFactor(climate, bazi) {
...
function checkWuxingPresence(bazi, wx) {
    if (!wx)
        return false;
...
// ---- 刑冲合害 Analysis ----
function analyzeInteractions(bazi, dmWx) {
    const branches = [
...
function analyzeThreeHarmonies(bazi, dmWx) {
    const branches = [
    ...
    for (const set of THREE_HARMONY_SETS) {
        ...
        if (branches.includes(a) && branches.includes(b) && branches.includes(c)) {
            ...
            factors.push({
                ...
                category: score >= 0 ? 'support' : 'weaken',
                score,
                description: `三合${set.wuxing}局，${score >= 0 ? '化合之气生扶日主' : '化合之气削弱日主'}`,
            });
        }
    }
        ...
        for (let j = i + 1; j < branches.length; j++) {
            ...
            if (half) {
                ...
                factors.push({
                    ...
                    category: score >= 0 ? 'support' : 'weaken',
                    score,
                    description: `半合${half.wuxing}局，${score >= 0 ? '半合之气生扶日主' : '半合之气削弱日主'}`,
                });
            }
        }
    }
    ...
    for (const set of THREE_MEETING_SETS) {
        ...
        if (branches.includes(a) && branches.includes(b) && branches.includes(c)) {
            ...
            factors.push({
                ...
                category: score >= 0 ? 'support' : 'weaken',
                score,
                description: `三会${set.wuxing}局（方局），${score >= 0 ? '方局之气生扶日主' : '方局之气削弱日主'}`,
            });
        }
    }
    return factors;
...
function scoreCombinedElement(combinedWx, dmWx, supportScore, weakenScore) {
    if (combinedWx === dmWx)
        return supportScore; // same → strong support
    if (generates(combinedWx, dmWx))
        return supportScore; // generates DM → support
    if (generates(dmWx, combinedWx))
        return -weakenScore; // DM generates → weaken
    if (controls(combinedWx, dmWx))
        return -(weakenScore + 2); // controls DM → heavy weaken
    if (controls(dmWx, combinedWx))
        return -(weakenScore - 2); // DM controls → mild weaken
    return 0;
}
// ---- Analyzers ----
function analyzeMonthOrder(dmWx, monthWx, monthName) {
...
function analyzeRoots(bazi, dm) {
    ...
    for (const p of pillars) {
        const hidden = HIDDEN_STEMS[p.idx];
        if (!hidden)
            continue;
...
function analyzeStemSupport(bazi, dmWx) {
    const pillars = [
...
function analyzeBranchSupport(bazi, dmWx) {
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
    let score = 0;
    const supporters = [];
    for (const p of pillars) {
...
function analyzeWeakening(bazi, dmWx) {
    const pillars = [
        { label: '年干', stem: bazi.year.stem.name, stemIdx: bazi.year.stemIndex },
        { label: '月干', stem: bazi.month.stem.name, stemIdx: bazi.month.stemIndex },
        { label: '时干', stem: bazi.hour.stem.name, stemIdx: bazi.hour.stemIndex },
...
// ---- Summary Builder ----
function buildSummary(dmName, dmWx, score, level, monthOrder, roots, stemSupport, weakening) {
    const parts = [];
...
//# sourceMappingURL=strengthEngine.js.map