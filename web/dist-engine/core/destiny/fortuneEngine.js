...
import { isClash, isCombination, getPunishment, isHarm, THREE_HARMONY_SETS, isStemClash, isStemCombine, } from '../astro/earthlyBranchRelations.js';
const LEVEL_LABELS = {
...
// ---- Main ----
export function analyzeFortune(bazi, strength, structure, climate, relations, dayun, liunian) {
...
// ---- Builders ----
function buildYearlyAnalysis(dayun, liunian, bazi) {
...
function computeYunSuiModifiers(dayun, liunian, bazi) {
    ...
    // Individual interactions
        ...
        for (const set of THREE_HARMONY_SETS) {
            const needed = set.branches.filter(b => b !== dyBranch && b !== lnBranch);
            if (needed.length === 1 && natalBranches.includes(needed[0])) {
                ...
                const isFavorable = combinedWx === dmWx ||
                    (combinedWx === '水' && dmWx === '木') ||
                    (combinedWx === '木' && dmWx === '火') ||
                    (combinedWx === '火' && dmWx === '土') ||
                    (combinedWx === '土' && dmWx === '金') ||
...
function buildOverallAssessment(yearly, structure, climate) {
    if (yearly.length === 0) {
        return {
...
function buildKeyYears(yearly) {
    if (yearly.length === 0)
        return { best: null, worst: null };
    let best = yearly[0];
    let worst = yearly[0];
    for (const y of yearly) {
        if (y.overall > best.overall)
            best = y;
        if (y.overall < worst.overall)
            worst = y;
    }
    return { best, worst };
}
function buildLifePeriods(dayun, bazi) {
...
function buildSummary(overall, structure, climate, relations, strength) {
    const parts = [];
    parts.push(`命局${structure.primaryPattern}，日主${strength.level}，运势整体处于${overall.level}期（${overall.score}分）。`);
    const dimLabelMap = { '事业': 'career', '财富': 'wealth', '感情': 'relationship', '健康': 'health' };
    const bestScore = overall.dimensions[dimLabelMap[overall.bestDimension]] ?? 0;
    const riskScore = overall.dimensions[dimLabelMap[overall.riskDimension]] ?? 0;
    ...
    if (relations.relations.length > 0) {
        ...
        if (favRel.length > 0)
            parts.push(`有利关系：${favRel.map(r => r.name).join('、')}。`);
        if (unfavRel.length > 0)
            parts.push(`需注意：${unfavRel.map(r => r.name).join('、')}。`);
    }
...
// ---- Helpers ----
function avg(yearly, key) {
    return Math.round(yearly.reduce((s, y) => s + y[key], 0) / yearly.length);
}
//# sourceMappingURL=fortuneEngine.js.map