...
function wuxingToShiShen(dmWx, targetWx, isYin) {
    if (targetWx === dmWx)
        return isYin ? '比肩' : '劫财';
    if (GENERATED_BY[targetWx] === dmWx)
        return isYin ? '正印' : '偏印';
    if (GENERATES[dmWx] === targetWx)
        return isYin ? '食神' : '伤官';
    if (CONTROLLED_BY[targetWx] === dmWx)
        return isYin ? '正官' : '七杀';
    if (CONTROLS[dmWx] === targetWx)
        return isYin ? '正财' : '偏财';
    return '比肩';
}
// ---- Main ----
export function deriveYongShen(bazi, strength, structure, climate) {
...
function deriveFollowWeakYongShen(bazi, dmWx) {
    ...
    if (cScore >= dScore && cScore >= coScore && cScore > 0)
        return controllerWx;
    if (dScore >= coScore && dScore > 0)
        return drainerWx;
    return consumerWx;
}
function checkWuxingInChart(bazi, wx) {
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
    return pillars.some(p => p.stem.wuxing === wx || p.branch.wuxing === wx);
}
function shiShenToWuxing(shiShen, dmWx) {
    switch (shiShen) {
        case '比肩':
        case '劫财':
            return dmWx;
...
function buildSummary(yongShen, xiShen, jiShen, chouShen, strength, climate) {
    const parts = [];
    parts.push(`日主${strength.dayMaster.wuxing}${strength.level}，` +
        `用神为${yongShen.wuxing}（${yongShen.shiShen}），${yongShen.reason}。`);
    parts.push(`喜神${xiShen.map(x => x.wuxing).join('、')}，` +
        `${xiShen.map(x => x.reason).join('；')}。`);
    parts.push(`忌神${jiShen.map(j => j.wuxing).join('、')}，` +
...
//# sourceMappingURL=yongShenEngine.js.map