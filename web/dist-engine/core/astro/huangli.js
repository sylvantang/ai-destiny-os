...
import { getHourStemStart, sexagenaryIndex, SEXAGENARY_NAMES, ALL_BRANCHES, } from './constants.js';
import { isClash, isCombination, getPunishment, isHarm, } from './earthlyBranchRelations.js';
// ---- 基础表 ----
...
export function getHuangli(year, month, day) {
    ...
    const jiShen = [];
    const xiongSha = [];
    if (TIANDE[monthBranch] === dayStemName)
        jiShen.push('天德');
    if (YUEDE[monthBranch] === dayStemName)
        jiShen.push('月德');
    if (TIANEN.has(dayGz))
        jiShen.push('天恩');
    if (TIANSHU[ALL_BRANCHES[monthBranch].name] === dayGz)
        jiShen.push('天赦');
    if (MUCANG[ALL_BRANCHES[monthBranch].name]?.includes(dayBranch))
        jiShen.push('母仓');
    if (SANHE[monthBranch]?.includes(dayBranch) && dayBranch !== monthBranch)
        jiShen.push('三合');
    if (isCombination(dayBranch, monthBranch))
        jiShen.push('六合');
    if (YIMA[yearBranch] === dayBranch)
        jiShen.push('驿马');
    if (isYellow)
        jiShen.push(zhishen); // 黄道值神入吉神
    if (dayBranch === (monthBranch + 6) % 12)
        xiongSha.push('月破');
    if (dayBranch === (yearBranch + 6) % 12)
        xiongSha.push('大耗');
    const sijiTarget = SIJI[ALL_BRANCHES[monthBranch].name];
    if (sijiTarget !== undefined && dayBranch === sijiTarget)
        xiongSha.push('四击');
    if (dayBranch === 5 || dayBranch === 11)
        xiongSha.push('重日');
    if (!isYellow)
        xiongSha.push(zhishen); // 黑道值神入凶煞
    // 胎神
