...
// ============================================================
import { getStem, getBranch, getHiddenStems, getNayin, sexagenaryIndex, getShiShen, } from './constants.js';
/**
 ...
 */
export function calcLiuNian(bazi, startYear, endYear) {
 ...
 */
export function calcLiuYue(bazi, year) {
    const dmIdx = bazi.day.stemIndex;
    const yearPillar = getYearPillar(year, dmIdx);
    ...
    for (let m = 0; m < 12; m++) {
        const branchIdx = ((m + 2) % 12); // 2=寅, 3=卯, ..., 1=丑
        const stemIdx = ((stemStart + m) % 10);
 ...
 */
export function calcLiuRi(date, dayMasterIndex) {
    ...
    return {
        ...
        hiddenStems: getHiddenStems(branchIdx),
        nayin: getNayin(dayIndex),
        shiShen: getShiShen(dayMasterIndex, stemIdx),
    };
}
// ---- Internal helpers ----
function getYearPillar(year, dmIdx) {
    // 公元4年为甲子年
    ...
    return {
        ...
        stemIndex: stemIdx,
        branchIndex: branchIdx,
        sexagenaryIndex: yearIndex,
        hiddenStems: getHiddenStems(branchIdx),
        nayin: getNayin(yearIndex),
        shiShen: getShiShen(dmIdx, stemIdx),
    };
}
/**
 ...
 */
function calcYearScores(bazi, yearPillar) {
    ...
    function applyShiShenEffect(shiShen, weight) {
 ...
 */
function getShiShenEffect(shiShen) {
    switch (shiShen) {
        // Self / peers
        case '比肩':
...
//# sourceMappingURL=liunian.js.map