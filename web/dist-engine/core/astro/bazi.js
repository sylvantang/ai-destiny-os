...
// ============================================================
import { getStem, getBranch, getHiddenStems, getNayin, getMonthStemStart, getHourStemStart, getShiShen, sexagenaryIndex, stemFromSexagenary, branchFromSexagenary, SEXAGENARY_NAMES, ALL_STEMS, ALL_BRANCHES, JIE_TO_MONTH_BRANCH, } from './constants.js';
import { getJieQi } from './jieqi.js';
import { getHourBranch } from './solarTime.js';
import { calcDaYun, getCurrentDayun } from './dayun.js';
import { isClash, isCombination, getPunishment, isHarm, isStemClash, getStemClashName, isStemCombine, } from './earthlyBranchRelations.js';
...
function birthEpoch(birth) {
    return (Date.UTC(birth.year, birth.month - 1, birth.day, birth.hour, birth.minute)
        - CST_OFFSET_MS);
}
/**
...
function calcHourPillar(birth, epoch, dayStem, dmIdx) {
    ...
    const hourBranch = getHourBranch(new Date(epoch), birth.longitude, birth.isDST, birth.standardMeridian ?? 120);
...
function summarizeRelations(bazi) {
    ...
    for (let i = 0; i < 4; i++) {
        for (let j = i + 1; j < 4; j++) {
            ...
            if (pun)
                branchPunishments.push(`${tag} ${pun}`);
            ...
            if (comb)
                stemCombines.push(`${tag} ${comb.name}`);
        }
    }
    return {
        stemClashes,
