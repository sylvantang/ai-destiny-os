...
function formatHiddenStemsRow(bazi) {
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
    return pillars.map(p => {
...
const STEM_NAMES_MAP = {
    0: '甲', 1: '乙', 2: '丙', 3: '丁', 4: '戊', 5: '己', 6: '庚', 7: '辛', 8: '壬', 9: '癸',
};
function formatNayinRow(bazi) {
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
    return pillars.map(p => {
...
function formatShiShenRow(bazi) {
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
...
function formatWuxingCount(chart) {
...
function formatDayMasterInfo(chart) {
...
export function colorWx(wx, text) {
    return (WX_COLORS[wx] ?? ((s) => s))(text);
}
/**
 * Render a compact single-line chart summary.
 */
export function renderChartSummary(chart) {
    const { bazi } = chart;
    const pillars = [bazi.year, bazi.month, bazi.day, bazi.hour];
 ...
 */
export function renderChartPlain(chart) {
    // Strip ANSI codes
