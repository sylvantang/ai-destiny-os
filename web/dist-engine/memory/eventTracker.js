 ...
 */
export function trackEvent(store, params) {
 ...
 */
export function getLifeTimeline(store) {
...
function summarizeYear(year, events) {
    if (events.length === 0)
        return `${year}年：无记录事件`;
 ...
 */
export function detectPatterns(store) {
    const events = store.getTimeline();
    if (events.length < 2)
        return [];
    const patterns = [];
 ...
 */
export function assessCurrentPhase(store) {
    const recent = store.getRecentEvents(8);
    if (recent.length === 0)
        return '数据积累期 — 尚未有足够的生活事件记录';
 ...
 */
export function buildMemoryContext(store) {
...
function computeAccuracyTrend(store) {
