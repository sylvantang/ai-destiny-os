...
export const searchMemoryTool: ToolDefinition = {
  ...
  async execute(params: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    if (params.startYear) filter.startYear = params.startYear;
    if (params.endYear) filter.endYear = params.endYear;
    if (params.minImpact) filter.minImpact = params.minImpact;

    const events = context.memory.getEvents(filter as any);
    const predictions = context.memory.getPredictionsByYear(
      (params.startYear as number) ?? new Date().getFullYear()
    );
    ...
    const lines: string[] = [`搜索"${params.query}"的结果：`, ''];
...
function mapQueryToDomain(query: string): string | null {
  const q = query.toLowerCase();
  if (q.includes('事业') || q.includes('工作') || q.includes('职业')) return '事业';
  if (q.includes('财') || q.includes('钱') || q.includes('收入')) return '财运';
  if (q.includes('感情') || q.includes('婚姻') || q.includes('恋爱')) return '感情';
  if (q.includes('健康') || q.includes('身体')) return '健康';
  return null; // 综合搜索
}
