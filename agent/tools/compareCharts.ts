...
export const compareChartsTool: ToolDefinition = {
  name: 'compare_charts',
  ...
  async execute(params: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    const otherBirth: BirthInfo = {
      year: params.otherYear as number,
      month: params.otherMonth as number,
      day: params.otherDay as number,
      hour: params.otherHour as number,
      minute: (params.otherMinute as number) ?? 0,
      longitude: 116.4,
      isDST: false,
      gender: (params.otherGender as '男' | '女') ?? '男',
    };
    ...
    const wuxingCompat = computeWuxingCompat(
      self.ctx.strength.dayMaster.wuxing,
      other.ctx.strength.dayMaster.wuxing,
    );
...
function computeWuxingCompat(a: string, b: string): string {
  ...
  if (a === b) return '五行相同 — 相互理解但缺乏互补';
  if (generates[a] === b) return `${a}生${b} — 本人滋养对方`;
  if (generates[b] === a) return `${b}生${a} — 对方滋养本人（有利）`;
  if (controls[a] === b) return `${a}克${b} — 本人克制对方`;
  if (controls[b] === a) return `${b}克${a} — 对方克制本人（需注意）`;
  return '五行关系中性';
}

function checkYongShenCompatibility(a: string, b: string): string {
  const generates: Record<string, string> = { '木': '火', '火': '土', '土': '金', '金': '水', '水': '木' };

  if (a === b) return '用神相同，目标一致，合作关系良好';
  if (generates[a] === b) return '本人用神生对方用神，本人能帮助对方发展';
  if (generates[b] === a) return '对方用神生本人用神，对方能帮助本人发展';
