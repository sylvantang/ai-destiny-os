...
import type { LLMClient, ChatMessage, ToolCall } from '../llmClient.js';
...
export class LLMProvider implements ResponseProvider {
  ...
  constructor(private llm: LLMClient, toolContext?: ToolContext) {
   ...
   */
  private async resolveToolCallsNonStreaming(
    messages: ChatMessage[],
  ): Promise<ChatMessage[]> {
    const workingMessages = [...messages];
    const tools = this.toolContext ? toolsToOpenAIFormat() : undefined;
