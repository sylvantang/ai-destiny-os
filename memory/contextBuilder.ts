 ...
 */
export function formatMemoryForPrompt(ctx: MemoryEnrichedContext): string {
  const parts: string[] = [];
...
function formatAccuracy(score: number): string {
  if (score >= 1.5) return '非常准确';
  if (score >= 0.5) return '比较准确';
  if (score >= -0.5) return '基本准确';
  if (score >= -1.5) return '不太准确';
  return '需要大幅改进';
}
 ...
 */
export function buildPersonalizedOverlay(store: MemoryStore): string {
  ...
  parts.push(`用户已记录${stats.totalEvents}个生活事件，追踪时间跨度${
    stats.yearRange ? `${stats.yearRange[0]}-${stats.yearRange[1]}年` : '暂无数据'
  }。`);
 ...
 */
export function suggestToneAdjustment(store: MemoryStore): 'encouraging' | 'balanced' | 'cautionary' {
  const stats = store.getStats();
  ...
  if (negativeCount >= 3) return 'encouraging';
  if (negativeCount === 0) return 'balanced';
