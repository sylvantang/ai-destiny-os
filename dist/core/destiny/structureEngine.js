 ...
 */
export function analyzeStructure(bazi, strength) {
    ...
    const subPattern = monthHidden.length > 1
        ? shiShenToPattern(getShiShen(dm, monthHidden[1].stem))
        : null;
...
function buildFromWeak(bazi, dm, analysis) {
...
function buildFromStrong(_bazi, dm, analysis) {
