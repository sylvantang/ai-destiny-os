 ...
 */
export function renderFortuneTimeline(fortune) {
 ...
 */
export function renderDayunCycles(dayun) {
    if (dayun.length === 0)
        return '大运数据不足';
    const lines = [];
    lines.push('┌──────┬──────────┬───────────┬───────────┬───────────┬──────┐');
    ...
    for (const dy of dayun.slice(0, 8)) {
        ...
        lines.push(`│ ${String(dy.startAge).padStart(2)}岁 ` +
 ...
 */
export function renderLifePeriods(fortune) {
 ...
 */
export function renderYearCard(year) {
    const dims = [
        ['事业', year.career, '💼'],
        ['财富', year.wealth, '💰'],
...
//# sourceMappingURL=fortuneTimeline.js.map