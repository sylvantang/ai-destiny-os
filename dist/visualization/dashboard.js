 ...
 */
export function renderDashboard(chart, strength, structure, climate, relations, fortune, options = {}) {
    const sections = [];
    // 1. Header
...
function renderChartCompact(chart) {
    const { bazi } = chart;
...
// ---- Key Metrics ----
function renderKeyMetrics(strength, fortune, structure) {
...
// ---- Strength Gauge ----
function renderStrengthGauge(strength) {
...
// ---- Structure & Climate ----
function renderStructureClimate(structure, climate) {
...
// ---- Fortune Section ----
function renderFortuneSection(fortune) {
    const years = fortune.yearlyAnalysis;
    if (years.length === 0)
        return '运势数据不足，无法生成时间线。';
    // Current year focus
...
// ---- AI Section ----
function renderAISection(chart, strength, fortune, options) {
...
// ---- Memory Section ----
function renderMemorySection(options) {
