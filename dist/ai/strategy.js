 ...
 */
export function analyzeStrategy(ctx, personality, career, relationship, question) {
...
// ---- Location Analysis ----
function analyzeLocations(chart, climate, strength) {
    ...
    if (climate.needsAdjustment && climate.neededWuxing) {
        const neededWx = climate.neededWuxing;
    ...
    for (const [name, dir, wx, reason] of directions) {
        if (advice.some(a => a.direction === dir))
            continue; // already added from climate
...
// ---- Life Phase Analysis ----
function analyzeCurrentPhase(chart, fortune, _strength) {
...
// ---- Action Plan ----
function buildActionPlan(_chart, fortune, career, relationship) {
    const now = new Date();
...
function buildYearActions(year, bestDim, riskDim, career, relationship, _offset) {
    const actions = [];
    // Career actions
    if (bestDim === '事业') {
...
// ---- Decision Framework ----
function buildDecisionFramework(chart, personality) {
...
function actionPlanNote(plan, currentYear) {
...
export function renderStrategyProse(result, _ctx) {
    const paragraphs = [];
    // 1. Current phase
    paragraphs.push('人生战略的核心是"在对的时间做对的事"。' +
    ...
    // 5. Closing
    paragraphs.push('战略不是一成不变的蓝图，而是一套动态的决策原则。' +
