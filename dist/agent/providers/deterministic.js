...
export class DeterministicProvider {
    name = 'deterministic';
    respond(topic, state, chart, _prompt) {
        ...
        switch (topic) {
            ...
            case '运势': {
                const f = ctx.fortune;
                const yearlyText = f.yearlyAnalysis.slice(0, 3).map(y => `${y.year}年：事业${y.career}分 财富${y.wealth}分 感情${y.relationship}分 健康${y.health}分`).join('\n');
                return {
            ...
            case '综合':
            default: {
                const viz = renderDashboard(ctx.chart, ctx.strength, ctx.structure, ctx.climate, ctx.relations, ctx.fortune, { compact: true });
                const parts = [
